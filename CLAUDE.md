...
- **Мультитенант**: у каждого пользователя своя рабочая папка `tenants/<имя>/` (а до
  миграции — общая `data/`). Везде ниже «`data/`» означает рабочую папку ТЕКУЩЕГО
  пользователя; `mem_*`-инструменты сами пишут в неё (изоляция через контекст тенанта).
